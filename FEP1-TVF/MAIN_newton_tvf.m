...
meshes={'mesh1_3.mat';'mesh1_4.mat';'mesh1_5.mat'};%'mesh1_4.mat';'mesh1_5.mat'};%'mesh1_6.mat'}; 
...
%    Ndt(imesh) = ceil(T/h(imesh));
    ...
    MAXW11error(imesh) = max(W11error);
...
legend('L^{\infty}L^2 error','Slope equal to 1','Location','southeast');
grid on
subplot(2,1,2);
loglog(h,L2W11error,'*-');
...
hold on
loglog(h,h);
title('Errors in $L^2L^{1}$ norm on the gradients','Interpreter','latex')
xlabel('$h$','Interpreter','latex')
ylabel('$\left\| \nabla u-\nabla \overline{u} \right\|_{L^2(L^1)}$','Interpreter','latex')
legend('L^2W^{1,1} error','Slope equal to 1','Location','southeast')
grid on
