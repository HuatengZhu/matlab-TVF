...
%    Ndt(imesh) = ceil(T/h(imesh));
    ...
    for idt = 1 : Ndt(imesh)
        ...
        while ( iter < itermax && res > tol && relax > relaxmin )
            ...
            %% Residues
            % res = norm(X - X_pre,2) / norm(X_pre,2); %Residue 1: L2 norm of difference of current and previous time-step solution
            % res = norm(X - X_pre,inf) / norm(X_pre,inf);%Residue 2: L infinity norm of difference of current and previous time-step solution
